...
void clFuntion() {
	...
	pAMD.GPA_OpenContext(cmdQueueA);
	...
	if (1) {
		...
		while (!readyResult) {       // skipping a session which got overwritten
...
int main(int argc, const char * argv[]) {
#ifdef AMPPERAPIEnable
	PerformanceAMD pAMD;
	ckE(pAMD.GPA_Initialize(), __LINE__);
#endif
	if (argc == 5) {
		if (!strcmp(argv[1], "-P")) {
	...
	std::thread t1(clFuntion);
	std::thread t2(clFuntion);
	t1.join();
	t2.join();
#ifdef AMPPERAPIEnable
